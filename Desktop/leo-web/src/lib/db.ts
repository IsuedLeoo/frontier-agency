  ...
  exec(sql: string) {
    ...
    statements.forEach(statement => {
      try {
        ...
        console.warn(`SQL execution warning: ${e.message}`);
      }
    });
  }
...
export function initializeD1(db: D1Binding) {
...
export function getDatabaseQueries(db: D1Binding) {
  const database = new D1Database(db);
  return {
...
export function getRawD1(db: D1Binding) {
  return db;
}